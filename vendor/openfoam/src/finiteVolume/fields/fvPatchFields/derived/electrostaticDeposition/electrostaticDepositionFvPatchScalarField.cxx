...
) const
{
    ...
    return refConstCast<this_bctype>(fld.boundaryField()[patchi]);
}
...
void Foam::electrostaticDepositionFvPatchScalarField::setMaster() const
{
    ...
    const auto& bf = eV.boundaryField();
...
)
{
    ...
    if (Ceffptr_ && tiptf.Ceffptr_)
    {
        Ceffptr_->rmap(tiptf.Ceffptr_(), addr);
    }

    if (rptr_ && tiptf.rptr_)
    {
